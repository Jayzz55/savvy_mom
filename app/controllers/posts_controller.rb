class PostsController < ApplicationController
  def index
    ...
    @posts = @posts.where("saving > ?", params["min_saving"].to_f) if params["min_saving"].present?
    @posts = @posts.where("saving_percentage > ?", params["min_saving_percentage"].to_f) if params["min_saving_percentage"].present?
